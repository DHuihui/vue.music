<template>
	<transition name="songlist">	
		<div class="song-list" v-show="showState" @click.self="close()">
			...
			<div class="song-wrapper">
				<div class="content">
					<div class="list-con">
						<div class="songs">
							<ul>
								<router-link :to="'/play/'+ song.musicData.songmid+'/'+song.musicData.albummid" tag="div" v-for="(song,k) in getPlayList" :key="song.musicData.songid" @click.native="modifyPlayInfo(song,k)">
									<li :class="{active:getCurSongIndex==k}">
										<span class="song-name cur" v-text="song.musicData.songname"></span> 
			...
			<div class="close-list" @click.stop="close()">关闭列表</div>
		</div>
	</transition>
</template>
